...
function Cone() {
    ...
    for (var i = 0; i < discTriangles; i++){
        ...
        // drawing triangles centered at 0,0,0
        ...
        this.normals[i*6] = vec4(0,0,-1,0);
        this.normals[i*6+1] = vec4(0,0,-1,0);
        this.normals[i*6+2] = vec4(0,0,-1,0);


        // drawing triangles centered at 0,0,1
        ...
        this.normals[i*6+3] = normalize(vec4(((Math.cos(angle1)+Math.cos(angle2)) / 2), ((Math.sin(angle1) + Math.sin(angle2)) / 2), 1, 0));
        this.normals[i*6+4] = normalize(vec4(Math.cos(angle2), Math.sin(angle2), 1, 0)); 
        this.normals[i*6+5] = normalize(vec4(Math.cos(angle1), Math.sin(angle1), 1, 0)); 
