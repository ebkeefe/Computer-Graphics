...
function Cylinder() {
    ...
    for (var i = 0; i < discTriangles; i++){
        ...
        this.normals[i*12 + 6] = normalize(vec4(Math.cos(angle2), Math.sin(angle2), 0, 0));
        this.normals[i*12+7] = normalize(vec4(Math.cos(angle1), Math.sin(angle1), 0, 0)); 
        this.normals[i*12+8] = normalize(vec4(Math.cos(angle2), Math.sin(angle2), 0, 0)); 
        ...
        this.normals[i*12+9] = normalize(vec4(Math.cos(angle1), Math.sin(angle1), 0, 0));
        this.normals[i*12+10] = normalize(vec4(Math.cos(angle1), Math.sin(angle1), 0, 0)); 
        this.normals[i*12+11] = normalize(vec4(Math.cos(angle2), Math.sin(angle2), 0, 0)); 
